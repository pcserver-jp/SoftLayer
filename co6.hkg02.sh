...
sed -i -e '/^NOZEROCONF.*$/d'         /etc/sysconfig/network || $Error
sed -i -e '/^NETWORKING_IPV6.*$/d'    /etc/sysconfig/network || $Error
sed -i -e '/^IPV6INIT.*$/d'           /etc/sysconfig/network || $Error
sed -i -e '/^IPV6_AUTOCONF.*$/d'      /etc/sysconfig/network || $Error
sed -i -e '/^IPV4_FAILURE_FATAL.*$/d' /etc/sysconfig/network || $Error
...
EOF
sed -i -e '/^# Disable IPv6.*$/d'                     /etc/sysctl.conf || $Error
sed -i -e '/^net.ipv6.conf.all.disable_ipv6.*$/d'     /etc/sysctl.conf || $Error
sed -i -e '/^net.ipv6.conf.default.disable_ipv6.*$/d' /etc/sysctl.conf || $Error
